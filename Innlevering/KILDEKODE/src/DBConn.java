...
public abstract class DBConn {
    ...
    public String getOkterBetween(String date0, String date1, String name) {
        ...
        try {
            ...
            Statement stmt = this.conn.createStatement();
            System.out.println("Creating statement ...");
            date0 = date0.replaceAll("[-]", "/");
            date1 = date1.replaceAll("[-]", "/");

            String sql =
                "SELECT DISTINCT (treningsokt.oktid), navn, varighet, form, prestasjon, dato " +
                "FROM ovelse " +
                    "INNER JOIN treningsoktOvelse ON treningsoktOvelse.ovelseid = ovelse.ovelseid " +
                    "INNER JOIN treningsokt ON treningsokt.oktid = treningsoktOvelse.oktid " +
                    "WHERE ovelse.navn='" + name + "' AND (dato BETWEEN '" + date0 + "' AND '" + date1 + "')" +
                        ";";

            System.out.println(sql);
            ResultSet rs = stmt.executeQuery(sql);
            sb.append("Søkeresultat for " + name + ":\n");
            while (rs.next()) {
                sb.append(
                    rs.getString("dato") + ", Varighet: " +
    ...
    public String getOvelserMedApparat(int apparatID) {
        StringBuilder sb = new StringBuilder();
        try {
            this.connect();
            Statement stmt = this.conn.createStatement();
            ...
            String sql =
                ...
                "FROM apparat INNER JOIN ovelse ON apparat.apparatid = ovelse.apparatid " +
                "WHERE ovelse.apparatid = " + apparatID + ";";
            ...
            stmt.close();
            this.disconnect();
            sb.append(sb.toString().equals("") ? "Ingen øvelser." : "");
        } catch (Exception e) {
            sb.append(e.getMessage());
            e.printStackTrace();
        }
        ...
        try{
            ...
            while (rs.next()) {
                for (String col: columns) {
                    result.append(rs.getString(col.replaceAll("([() ])|(DISTINCT)", "")) + " ");
                }
                result.append("\n");
            }
